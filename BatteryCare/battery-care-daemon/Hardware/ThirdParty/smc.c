...
kern_return_t SMCWriteForced(const char* key, unsigned char byte_val, io_connect_t conn)
{
    SMCKeyData_t inputStructure;
    SMCKeyData_t outputStructure;
    ...
    inputStructure.key = _strtoul((char*)key, 4, 16);
    inputStructure.data8 = SMC_CMD_WRITE_BYTES;
    inputStructure.keyInfo.dataSize = 1;
    inputStructure.bytes[0] = byte_val;

    return SMCCall2(KERNEL_INDEX_SMC, &inputStructure, &outputStructure, conn);
}
...
kern_return_t SMCWriteSimple(char* key, unsigned char* bytes, int len, io_connect_t conn)
{
    ...
    // Read the key first to obtain its actual dataSize; SMCWriteKey2 requires an exact match.
    result = SMCReadKey2(key, &val, conn);
    if (result != kIOReturnSuccess) {
        return result;
    }

    // Overwrite only the bytes we were given; leave any extra bytes from the read intact.
    for (i = 0; i < len && i < (int)val.dataSize; i++) {
        val.bytes[i] = bytes[i];
    }
    strncpy(val.key, key, 4);
